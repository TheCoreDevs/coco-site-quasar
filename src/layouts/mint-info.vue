<template>
  <q-page-container id="mint" class="flex flex-center black-bottom-border column section-padding">
    <div class="text-uppercase text-h2">Minting</div>
    <div class="text-center fs-18 q-pt-md q-pb-md lighter">When you mint a cocoSwirl, you’ll receive a<br/> unique background NFT at random</div>
    <div class="q-mt-md dots full-width q-pt-lg q-pb-lg q-mb-lg">
      <swiper
        ...
        :spaceBetween="0"
        :centeredSlides="true"
        :autoplay="useAnimations ? {delay: 1500, disableOnInteraction: false} : false"
        :loop="true"
        :infinite="true"
      >
        <template v-for="i in 5" :key="`${i} key`">
          <swiper-slide>
            <q-img class="non-selectable" :src="`./mint_images/${i - 1}.png`" fit="contain" />
          </swiper-slide>
        </template>
      </swiper>
    </div>
    <div class="text-h5 font-mulish"><strong>{{amountMinted}} of {{amountMax}} remaining</strong></div>
    <div class="row q-pt-sm q-mb-md"><span class="text-uppercase fs-10 self-center">Price</span> <span class="fs-18 q-pl-sm"><strong>{{ethPrice}} ETH</strong></span></div>
    <q-btn rounded class="custom-btn full-width full-height" style="max-width: 250px; max-height: 50px" @click="startMint">
      <q-spinner v-if="isLoading" />
      <div v-show="!isLoading">Mint</div>
    </q-btn>
    <div class="text-center q-pt-md lighter">By minting, you agree to the <a href="/">Terms of Service</a>.</div>
    ...
    <q-dialog
      ...
      maximized
    >
      <div :class="`bg flex flex-center full-width ${$q.screen.gt.sm ? 'row no-wrap' : 'column reverse'}`">
        <q-card :class="`q-pa-md ${$q.screen.gt.sm ? 'q-mr-xl' : 'q-mt-lg'}`" style="border-radius: 15px">
          <q-card-section class="text-h5 font-mulish text-center"><strong>Mint a cocoSwirl</strong></q-card-section>
          <q-card-section class="font-mulish lighter fs-16 text-center q-pt-none">How many cocoSwirls would you like?</q-card-section>
          <q-card-section class="q-pl-none q-pr-none">
            <q-btn-dropdown flat class="full-width overlay-dropdown" :label="mintAmount" style="border-radius: 15px">
              <q-list class="overlay-list">
                <template v-for="i in 2" :key="i">
                  <q-item clickable v-close-popup @click="onItemClick(i)" :class="[`${i === mintAmount ? 'selected' : ''}`]">
                    <q-item-section>
          ...
          <q-card-section class="text-center font-mulish q-pt-none">
            <strong>{{amountMinted}}</strong> of <strong>{{amountMax}}</strong> remaining
          </q-card-section>
          <q-separator class="q-mt-lg" />
          <q-card-section>
            <div class="text-uppercase fs-10">Total</div>
            <div>
              <strong>{{(ethPrice * mintAmount).toFixed(3)}} ETH</strong> <span class="lighter q-pl-sm">~${{currentPriceTotal}} USD</span>
            </div>
          </q-card-section>
          <q-btn rounded class="custom-btn full-width q-mt-md" @click="attemptMint">
            Mint
          </q-btn>
        </q-card>
        ...
        <q-img :class="`mystery-img ${$q.screen.gt.sm ? 'q-ml-xl' : ''}`"
    ...
    <q-dialog
      ...
      maximized
    >
      <q-card class="q-pa-md text-center full-height" style="border-radius: 15px !important; max-width: 400px; width: 100%; max-height: 700px">
        <div class="text-h6 font-mulish"><strong>Thank you!</strong></div>
        <div class="fs-16 lighter q-pt-md">
          This might take a few minutes. Track your transaction on Etherscan, or check OpenSea to see if it’s complete.
        </div>
        <q-btn rounded class="custom-btn full-width invert q-mt-lg" :href="etherscanLink" target="_blank">
          Track on Etherscan
        </q-btn>
        <q-btn rounded class="custom-btn full-width q-mt-md" :href="openseaLink" target="_blank">
          View on OpenSea
        </q-btn>
        <q-separator class="q-mt-xl q-mb-lg" />
        <div class="text-h6 font-mulish"><strong>Swirl your PFP</strong></div>
        <div class="fs-16 lighter q-pt-md">
          Customize your profile picture by adding your new swirl to the background.
        </div>
...
import { getUsefulError, goto, reduceMotionEnabled, sleep } from "src/scripts/util";
...
import { mint1, mint2 } from "src/scripts/custom/main";
...
async function attemptMint() {
  ...
  try {
    if (mintAmount.value === 1) {
      output = await mint1(pair, price);
    } else {
      output = await mint2(pair, price);
    }
  } catch (e) {
    showError(e.message);
    return;
